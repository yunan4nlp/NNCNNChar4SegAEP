...
int Classifier::addTestAlpha(const vector<Instance>& vecInsts) {
	...
	for (int numInstance = 0; numInstance < vecInsts.size(); numInstance++) {
		...
		for (int i = 0; i < att_num; i++) {
			string curatt = normalize_to_lowerwithdigit(atts[i]);
			if (!m_options.attEmbFineTune)m_att_stats[curatt]++;
		}
...
void Classifier::train(const string& trainFile, const string& devFile, const string& testFile, const string& modelFile, const string& optionFile) {
	...
	m_att_stats[unknownkey] = m_options.attCutOff + 1;
	m_driver._modelparams.attAlpha.initial(m_att_stats, m_options.attCutOff);
	m_driver._modelparams.atts.initial(&m_driver._modelparams.attAlpha, m_options.attEmbSize, m_options.attEmbFineTune);
